    ...
    <div className="min-h-screen bg-[#f4f6f9]">
      <header className="sticky top-0 z-20 border-b border-gray-200/80 bg-white/90 backdrop-blur-md">
        <div className={`${PAGE_CONTAINER} flex h-13 items-center gap-4 mobile:h-12`}>
          <span className="text-sm font-bold tracking-tight text-gray-900 mobile:text-base">
            AppTrack
          </span>
          ...
          <div className="flex items-center gap-3">
            <span className="mobile:hidden hidden text-xs text-gray-400 sm:block">{email}</span>
            <form action={ROUTES.signOut} method="post">
              <button
                type="submit"
                className="cursor-pointer text-xs font-medium text-gray-500 transition-colors hover:text-gray-900 mobile:text-sm mobile:min-h-11 flex items-center"
              >
                Sign out
              </button>
            </form>
